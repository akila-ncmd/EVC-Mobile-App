...
The screenshots in this README are downscaled from those same goldens, so
regenerate them in the same pass — otherwise they keep advertising a version
of the UI that no longer exists:
...
Every image in `assets/images/` is an **original abstract composition**,
generated by [`scripts/generate_artwork.py`](scripts/generate_artwork.py) and
owned outright by this repository. Nothing is downloaded, traced, or derived
from an existing work, and the script is deterministic — re-running it
reproduces the same 30 files:

```bash
python scripts/generate_artwork.py
```

Each image is a mesh gradient in one of eight palette families with a single
geometric motif drawn over it. Family and motif are assigned per image in the
script's art-direction table rather than picked at random: random selection
clumped eight of thirty onto the same diagonal-stripe idea and let the palette
wander off-brand. Assigning them keeps the burgundy families dominant, puts
all ten motifs on screen, and gives titles that share a rail contrasting
artwork.

These replaced the film posters, album covers and photographs of real people
that were cropped from the original mockups, none of which could ship.

> **Titles and names are still placeholders.** The catalogue continues to
> carry film, album and artist names from those mockups. They belong to their
> respective owners, appear here only to populate a layout, and imply no
> affiliation or endorsement. Rename `MockData` before promoting this
> anywhere public.

Demo audio is royalty-free (SoundHelix); demo video is the Sintel trailer
